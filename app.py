...
def schema():
    ...
    # values[0][0]="columns"
    # print(values)
    schema_format = request.form.get('schematype')
    print(schema_format)
    if schema_format=='csv':
...
#for adding the required values to the concate date_time and regular datetime
    ...
    #writing the output on json file
        ...
        return send_from_directory(filepath,'RESULT34.json',as_attachment =True)
    elif schema_format=='fixedlength':
        schema = {"parser_settings": {
        "version": "omni.2.0",
        ...
        #file_declaration["envelopes"][0]["columns"]
        f= request.files['file']
        if not os.path.isdir('static'):
            os.mkdir('static')
        filepath =os.path.join('static','auto_schema1.csv')
        f.save(filepath)
        print(f.filename)
        values=[]
        row=0
        ...
        for i in value_dict:
            ...
            value_dict['statuscode']=[str(col_len[0]),re.sub('[^A-Za-z0-9]+', '',value_dict['statusdescription'][1].upper())]
        test_list = ['01/02/2006 15:04:05 PM','02/01/2006 3:04:05 AM','02-01-2006 3:04:05 AM','2006/01/02 3:04:05 AM','2006-01-02 3:04:05 AM','02/01/2006 3:04:05','2006-01-02T15:04:05-07:00','1585990763']
# Handling no element found in index()
# Using try + except + ValueError
        try :
            if value_dict['eventDateTime'][1]!="":
            ...
            print(e)
            print("The value after catching error : " + str(res))
        loc=final_output_event_obj['array'][0]['object']['location']['object']
        if value_dict['event_country_code'][0]!='':
            loc["country"]={ "xpath": "event_country_code" }
        if value_dict['event_state'][0]!='':
            loc["state"]={ "xpath": "event_state" }
        if value_dict['event_city'][0]!='':
            loc["city"]={ "xpath": "event_city" }
        if value_dict['event_zip'][0]!='':
            loc["zip"]={ "xpath": "event_zip" }
        if value_dict['long'][0]!='':
            loc["lng"]={"xpath": "long"}
        if value_dict['lat'][0]!='':
            loc["lat"]={"xpath": "lat"}
        add_values = dict(list(value_dict.items())[1:])
        new_dict = dict([(value[0], key) for key, value in add_values.items() if value[0]!=""])
        new_dict1=dict([(key,value[2]) for key, value in add_values.items() if value[0]!=""])
        new_dict2=dict([(key,value[3]) for key, value in add_values.items() if value[0]!=""])
        for key,value in new_dict.items():
            temp = {}
            temp["name"] = str(value)
            if temp["name"] in new_dict1:
                temp["start_pos"]=int(new_dict1[temp["name"]])
            if temp["name"] in new_dict2:
                temp["length"]=int(new_dict2[temp["name"]])
                if value_dict['line_pattern'][1]!='':
                    temp["line_pattern"]=value_dict['line_pattern'][1]
            file_declaration["envelopes"][0]["columns"].append(temp)
        carrier_template = {
            "const": ""
        }
        carrier_status_code_template= {
...
#for adding the required values to the concate date_time and regular datetime
        ...
        final_output_obj['object']["tracking_entity"] = trackingEntity
        transform_declarations['FINAL_OUTPUT']=final_output_obj
        transform_declarations['message_source_date']={
      "external": "message_source_date"
    }
        schema["transform_declarations"] = transform_declarations
        x=json.dumps(schema,indent=2)
        f = open("RESULT35.json",'w')
        f.write(x)
        f.close()
        filepath =os.path.join(app.root_path)
        #f.save(filepath)
        ...
        #return render_template('response.html', output_schema=x)
        return send_from_directory(filepath,'RESULT35.json',as_attachment =True)
...
"tracking_document": {
    "object": {
        ...
        if not os.path.isdir('static'):
            os.mkdir('static')
        filepath =os.path.join('static','auto_schema2.csv')
        f.save(filepath)
        print(f.filename)
        values=[]
        row=0
...
#for capitalize and removing special characters from statuscode
        for i in value_dict:
            ...
            value_dict['statuscode']=[str(col_len[0]),re.sub('[^A-Za-z0-9]+', '',value_dict['statusdescription'][1].upper())]
        test_list = ['01/02/2006 15:04:05 PM','02/01/2006 3:04:05 AM','02-01-2006 3:04:05 AM','2006/01/02 3:04:05 AM','2006-01-02 3:04:05 AM','02/01/2006 3:04:05','2006-01-02T15:04:05-07:00','1585990763','2006-01-02 15:04:05']
# Handling no element found in index()
# Using try + except + ValueError
        try :
            if value_dict['eventDateTime'][1]!="":
        ...
        loc=final_output_event_obj['array'][0]['object']['location']['object']
        if value_dict['event_country_code'][0]!='':
            loc["country"]={ "xpath": value_dict['event_country_code'][0] }
        if value_dict['event_state'][0]!='':
            loc["state"]={ "xpath": value_dict['event_state'][0] }
        if value_dict['event_city'][0]!='':
            loc["city"]={ "xpath": value_dict['event_city'][0] }
        if value_dict['event_zip'][0]!='':
            loc["zip"]={ "xpath": value_dict['event_zip'][0] }
        if value_dict['long'][0]!='':
            loc["lng"]={"xpath": value_dict['long'][0]}
        if value_dict['lat'][0]!='':
            loc["lat"]={"xpath": value_dict['lat'][0]}
        add_values = dict(list(value_dict.items())[1:])
        new_dict = dict([(value[0], key) for key, value in add_values.items() if value[0]!=""])
        final_output_obj['xpath']=add_values['root'][0]
        carrier_template = {
            "const": ""
        }
        carrier_status_code_template= {
...
#for adding the required values to the concate date_time and regular datetime
        ...
        try:
            ...
            print(e)
            print("Please specify time in csv.")
        if value_dict['tracking_number'][0]!='':
            trackingEntity['object']['tracking_number']['xpath']=value_dict['tracking_number'][0]
            trackingEntity['object']['tracking_document']['object']['tracking_number']['xpath']=value_dict['tracking_number'][0]

        for key in add_values.keys():
            if 'tracking_number' in key:
                trackingEntity['object']['tracking_number']['xpath'] = add_values[key][0]
                trackingEntity['object']['tracking_document']['object']['tracking_number']['xpath'] =add_values[key][0]
            if 'statuscode' in key:
                if add_values['statuscode'][0]=="":
                    carrier_status_code_template['custom_func']['args'][0]['custom_func']['args'][0]['xpath']=add_values[key][0]
                    final_output_event_obj['array'][0]['object']['carrier_status_code']['template']="carrier_status_code_template"
                else:
                    carrier_status_code_template['custom_func']['args'][0]['custom_func']['args'][0]['xpath']=add_values[key][0]
                    final_output_event_obj['array'][0]['object']['carrier_status_code']['template']="carrier_status_code_template"
            if 'statusdescription' in key:
                final_output_event_obj['array'][0]['object']['desc']['xpath']=add_values[key][0]
            if 'eventDateTime' in key:
                event_date_time["custom_func"]["args"][0]["xpath"] = add_values[key][0]
     
            if 'eventdate' in key:
                concate_event_date_time['custom_func']['args'][0]['custom_func']['args'][0]['xpath']=add_values[key][0]
            if 'eventtime' in key:
                concate_event_date_time['custom_func']['args'][0]['custom_func']['args'][1]['xpath']=add_values[key][0]
        if value_dict['statusdescription'][0]=="":
            del final_output_event_obj['array'][0]['object']['desc']['xpath']
            final_output_event_obj['array'][0]['object']['desc']=carrier_description
        import json
      ...
      "external": "message_source_date"
        ...
        #return render_template('response.html', output_schema=x)
        return send_from_directory(filepath,'RESULT36.json',as_attachment =True)
